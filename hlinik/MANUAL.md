...
IRIS je název LARPu. HLINÍK je fiktivní firma sídlící v Humpolci, která má přelomový AI projekt — IRIS-app, poskytující AI co nejpodobnější člověku. Jádro hry je obrácený Turingův test: agenti (lidé) předstírají, že jsou AI, a uživatelé se snaží odhalit, že mluví s člověkem.

### 1.1 Role

| Role | Uživatelé | Popis |
|------|-----------|-------|
| **Uživatel (User)** | user1–user8 | Dostává úkoly. Plní je tak, že je zadává agentům, ti vrací výsledky, a uživatel postne text od "AI" jako splněný úkol. Za to dostává kredity. Reportuje zprávy, které vypadají jako od AI — úspěšný report = odměna v kreditech. |
| **Agent (Operátor)** | agent1–agent8 | Tváří se jako AI. Je to člověk, který odpovídá uživatelům a tvrdí o sobě, že je umělá inteligence. Co neví, musí si vymyslet. Může být zastoupen LLM (autopilot), ale pokud uživatel zprávu od LLM reportuje, agent dostává záporné body. |
| **Správce (Admin)** | admin1–admin4 | Dohlíží na systém, ovládá jej, vyhodnocuje úkoly. Ovládací panel má záměrně nesmyslné popisky (jazykový mód od ROOTa), správci si je mohou sami přejmenovat (vyšší priorita než nastavení roota). Panely vizuálně představují panely z různých sérií Star Trek. |
| **ROOT** | root | Gamemaster. Nastavuje jazyky, jména a hesla, kontroluje parametry systému. Může dávat odměny/tresty a psát hromadné zprávy. Má přístup ke všemu + konfigurace AI, server restart, factory reset. |
...
### 1.3 Sanity mechanismus

Systém si hlídá svou sanity. Pokud přijde hodně dobrých reportů od uživatelů, systém se resetuje — switchne směny (agenty a uživatele mezi sebou), čímž se provedou resety konverzací.

### 1.4 Technologie

- Backend: FastAPI (Python), SQLite/MySQL, WebSocket
...
- **ITERACE** — aktuální shift (0–7), určuje přiřazení agenta
...
- **Tlačítko [ ! ] (Report)** — nahlásí zprávu agenta jako anomálii. Každý report zvýší teplotu systému o 15°C. Zprávy s badge **VERIFIED** (přepsané optimizerem) nelze reportovat. Za úspěšný report dostane uživatel odměnu v kreditech.
...
**Penalizace:** Pokud uživatel úspěšně reportuje zprávu vygenerovanou LLM (autopilot), agent dostává záporné body.
...
Modrý/cyan terminál. Panely vizuálně představují panely z různých sérií Star Trek. Hub se 4 stanicemi.
...
|---|---------|-------------|-------|--------|
| 01 | MONITORING | UMYVADLO | Zelená | Vizualizace propojení agentů a uživatelů, log všech chatů |
| 02 | KONTROLNÍ PANEL | ROZKOŠ | Žlutá | Sledování sanity, možnost provést manuálně reset |
| 03 | EKONOMIKA | BAHNO | Modrá | Mění sociální status, přehled o kreditech |
| 04 | ÚKOLY | MRKEV | Fialová | Zadávání a schvalování splněných úkolů |

**Nesmyslné popisky:** Existuje jazykový mód pro nesmyslné popisky, který ROOT přepíná (viz sekce 6.3). Kódové názvy stanic jsou výchozí příklad. Správci si mohou všechny popisky v UI sami přejmenovat přes "PŘEPSAT REALITU" — úpravy správců mají **vyšší prioritu** než jazykové nastavení od ROOTa.

**Vizuální panic mode:** Při dosažení kritických parametrů (teplota > 350°C) se aktivuje vizuální panic mode na celém dashboardu.

### 5.2 Stanice MONITORING

Vizualizace propojení agentů a uživatelů + log všech chatů. 4 záložky:
...
### 5.3 Stanice KONTROLNÍ PANEL

Sledování sanity systému a možnost provést manuální reset (shift směn).

**POSUN REALITY (Shift):**
- Zobrazuje aktuální shift
- Tlačítko **ROZTOČIT KOLA OSUDU** — zvýší shift o 1 (provede reset — switchne agenty a uživatele, resetuje konverzace)
...
Zlatý/černý dashboard. Gamemaster — nastavuje jazyky, jména a hesla, kontroluje parametry systému, dává odměny/tresty, píše hromadné zprávy. Má všechny admin funkce + navíc:
...
- Výdělky: za splněné úkoly (odměna × rating%, minus daň), za úspěšné reporty (výchozí 25 CR)
...
SQLite (lokální vývoj) nebo MySQL (produkce). Konfigurace přes `DATABASE_URL` v `.env`. Modely:
...
# Databáze — MySQL pro produkci, SQLite jako fallback pro lokální vývoj
DATABASE_URL=mysql+pymysql://user:password@host:3306/iris?charset=utf8mb4
...
2. **Token bez revokace** — zamčený/smazaný user zůstane přihlášený do expirace (24h)
3. **Žádný rate limiting** na LLM API
